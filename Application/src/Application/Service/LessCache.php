...
class LessCache {
	...
	public static function Get( $less_files, $parser_options = array(), $modify_vars = array() ){
		...
		if( isset($parser_options['cache_dir']) ){
			LessCache::$cache_dir = $parser_options['cache_dir'];
		}

		if( empty(LessCache::$cache_dir) ){
		...
		if( !empty($modify_vars) ){
			...
			$vars_file = LessCache::$cache_dir.'lessphpvars_' . sha1($lessvars) . '.less';
		...
		$hash = md5(json_encode($less_files));
 		$list_file = LessCache::$cache_dir.'lessphp_'.$hash.'.list';
	...
	public static function Cache( &$less_files, $parser_options = array() ){
		...
		$parser_options['cache_dir'] = LessCache::$cache_dir;
	...
	private static function OutputFile( $compiled_name, $parser_options ){
		...
		if( !empty($parser_options['output']) ){
			...
			return LessCache::$cache_dir.$parser_options['output'];
		}

		return LessCache::$cache_dir.$compiled_name;
	}
	...
	public static function SetCacheDir( $dir ){
		LessCache::$cache_dir = $dir;
	}

	public static function CheckCacheDir(){

		LessCache::$cache_dir = str_replace('\\','/',LessCache::$cache_dir);
		LessCache::$cache_dir = rtrim(LessCache::$cache_dir,'/').'/';

		if( !file_exists(LessCache::$cache_dir) ){
			if( !mkdir(LessCache::$cache_dir) ){
				throw new \Less_Exception_Parser('Less.php cache directory couldn\'t be created: '.LessCache::$cache_dir);
			}

		}elseif( !is_dir(LessCache::$cache_dir) ){
			throw new \Less_Exception_Parser('Less.php cache directory doesn\'t exist: '.LessCache::$cache_dir);

		}elseif( !is_writable(LessCache::$cache_dir) ){
			throw new \Less_Exception_Parser('Less.php cache directory isn\'t writable: '.LessCache::$cache_dir);
	...
	public static function CleanCache(){
		...
		$files = scandir(LessCache::$cache_dir);
		if( $files ){
			...
			foreach($files as $file){
				...
				$full_path = LessCache::$cache_dir.'/'.$file;
